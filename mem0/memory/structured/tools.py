...
class TopicUpdateTool:
    ...
    def __call__(
        ...
        *,
        summary_overrides: Optional[Mapping[tuple[str, str], str]] = None,
    ) -> List[str]:
        memory_ids: List[str] = []
        overrides = dict(summary_overrides or {})
        updates = list(updates)
        if not updates and not overrides:
            return memory_ids
        ...
        topic_texts = [f"{topic}::{subtopic}" for topic, subtopic in grouped] if grouped else []
        ...
        for key in set(list(grouped.keys()) + list(overrides.keys())):
            topic_id = topic_ids.get(key)
            topic_name, subtopic_name = key
            if topic_id is None:
                topic_id = self.db.upsert_topic(topic=topic_name, subtopic=subtopic_name)
                topic_ids[key] = topic_id
            previous_summary = self.db.get_topic_summary(topic_id)
            if key in overrides:
                new_summary = overrides[key]
            else:
                group_updates = grouped.get(key, [])
